...
class PTrainer_augmented(Trainer):
    def __init__(self, training_params, model, data, device, log_wandb=True):
        super(PTrainer_augmented, self).__init__(training_params, model, data, device, log_wandb)
        self.val_interval = training_params['val_interval']
    ...
    def train(self, model_state=None, opt_state=None, start_epoch=0):
        ...
        for epoch in range(self.training_params['nr_epochs']):
            ...
            for data in self.train_ds:
                ...
                origin = data[1].to(self.device)
                transformed_origin = self.transform(origin) if self.transform is not None else images
                ...
                with autocast(enabled=True):
                    ...
                    target = transformed_origin if self.model.prediction_type == 'sample' else noise
                    loss = self.criterion_rec(pred.float(), target.float())
